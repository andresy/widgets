...
Window.__init = argcheck{
   ...
   call =
      function(self, parent, attr)
         ...
         self:__initAttributes({width=-1, height=-1, fullscreen=false, title="", resizable=false, color={r=0.9, g=0.9, b=0.9}})
...
function Window:sizeRequest()
   ...
   elseif self.attr.width and self.attr.height then
...
function Window:setGeometry(x, y, w, h)
   if self.drv.window then
      if x >= 0 and y >= 0 then
         sdl.setWindowPosition(self.drv.window, x, y) -- faudrait pas juste un setPosition pour window?
      end
   ...
   Widget.setGeometry(self, 0, 0, w, h)
   self.flags.updategeometry = false
end

Window.updateGeometry = argcheck{
   {name="self", type="gui.Window"},
   call =
      function(self)
...
Window.isGeometryReady = argcheck{
   {name="self", type="gui.Window"},
   call =
      function(self)
